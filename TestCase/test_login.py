...
from api.test import Transfertodo
...
class TestLOGIN(object):
    def setup(self):
        ...
        self.Transfertodo = Transfertodo()
    ...
    @allure.description('login')
    @pytest.mark.parametrize('case', load_data("Transfertodo", "/TestCase/test_login.yaml"))
    def test_Transfertodo(self, case):
        self.log.info("*************** 开始执行用例 ***************")
        case = replace_case_by_regular(case)
        self.log.info("用例名称  ==>> {}".format(case['test_name']))
        result = self.Transfertodo.page(case['needCode'])
        parser(result, case['test_name'], case['parser'], case['expected'])
        # 如果check_sql有值，说明要做数据库校验。
        # if case["check_sql"]:
        #     self.Assert.assert_sql(case["check_sql"])
